  ...
  return <>
        <div className='relative mx-auto hidden lg:block'>
            <button
                ...
                onClick={()=> {hasNext ?setPage(prev => prev + 1):hasPrev? setPage(prev => prev - 1): ''}}>
                <span className='pb-1'>{hasNext ? '>': hasPrev ? '<' : ''}</span>
            </button>               
            <ul className='flex gap-x-2'>
                {categories.slice(startIndex, endIndex).map((category, index)=>
                <button onClick={()=>setCategory(category.link)} className='px-2 py-3 rounded-t-2xl hover:bg-main/[.08] group relative' key={index}>
